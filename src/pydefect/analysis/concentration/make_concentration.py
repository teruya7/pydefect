...
from pydefect.analysis.concentration.concentration import DefectConcentration, \
...
class TotalDos(MSONable, ToJsonFileMixIn):
    ...
    Example:
        >>> total_dos = TotalDos.from_json_file()
        >>> print(total_dos.band_gap)
    """
    ...
    def __str__(self):
        ...
        table_data = []
        for energy, dos_val in zip(self.energies, self.dos):
            table_data.append([energy, dos_val])
        result.append(tabulate(table_data, headers=["energy", "dos"],
                               floatfmt=".2e"))
        return "\n".join(result)
