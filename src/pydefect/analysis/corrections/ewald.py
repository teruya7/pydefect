...
def grid_number(lattice_vectors: np.ndarray, max_length: float):
    """Calculate total grid points for Ewald sum."""
    grid_counts = [ceil(max_length / norm(lattice_vectors[axis])) for axis in range(3)]
    return grid_counts[0] * grid_counts[1] * grid_counts[2]


class Ewald(MSONable):
    """Anisotropic Ewald summation for electrostatic corrections.

    Calculates electrostatic interactions in a periodic system
    using the Ewald summation technique with anisotropic dielectric.

    Attributes:
        lattice: Real-space lattice vectors (3x3 numpy array).
        rec_lattice: Reciprocal lattice vectors.
        volume: Unit cell volume.
        dielectric_tensor: 3x3 dielectric tensor.
        ewald_param: Ewald parameter controlling real/reciprocal split.
        accuracy: Product of G-vector cutoff and Gaussian width.
        mod_ewald_param: Modified Ewald parameter (gamma in YK2014).
        diff_pot: Correction for finite Gaussian charge.

    Example:
        >>> import numpy as np
        >>> lattice = np.eye(3) * 10.0
        >>> dielectric = np.eye(3) * 10.0
        >>> ewald = Ewald(lattice, dielectric)
        >>> energy = ewald.lattice_energy
    """
    ...
    def __init__(self,
        ...
        Args:
            lattice: 3x3 real-space lattice vectors.
            dielectric_tensor: 3x3 dielectric tensor.
            accuracy: Accuracy parameter for cutoff. Higher is more accurate.
                Defaults to pydefect defaults.ewald_accuracy.
            ewald_param: Manual Ewald parameter. If None, calculated automatically.
        """
        ...
        if ewald_param:
            ...
            real_lattice_mean = mstats.gmean([norm(vector) for vector in self.lattice])
            rec_lattice_mean = mstats.gmean([norm(vector) for vector in self.rec_lattice])
            self.ewald_param = sqrt(rec_lattice_mean / real_lattice_mean / 2) * self.cube_root_vol / self.root_epsilon
    ...
    def ewald_real(self, include_self: bool, shift: List[float]) -> float:
        """Calculate real-space contribution to Ewald sum."""
        summed = 0
        for real_vector in self.r_lattice_set(include_self, shift):
            root_r_inv_epsilon_r = sqrt(reduce(dot, [real_vector.T, self.epsilon_inv, real_vector]))
    ...
    def ewald_rec(self, coord) -> float:
        """Calculate reciprocal-space contribution to Ewald sum."""
        cart_coord = dot(coord, self.lattice)
        summed = 0
        for g_vector in self.g_lattice_set():
            g_epsilon_g = reduce(dot, [g_vector.T, self.dielectric_tensor, g_vector])
            summed += (exp(- g_epsilon_g / 4 / self.mod_ewald_param ** 2)
                       / g_epsilon_g * cos(dot(g_vector, cart_coord)))
    ...
    def r_vector_nums(self) -> List[int]:
        """Calculate number of real-space vectors needed."""
        max_length = self.accuracy / self.mod_ewald_param
        return [ceil(max_length / norm(self.lattice[axis])) for axis in range(3)]

    @property
    def g_vector_nums(self) -> List[int]:
        """Calculate number of reciprocal-space vectors needed."""
        max_length = 2 * self.mod_ewald_param * self.accuracy
        return [ceil(max_length / norm(self.rec_lattice[axis])) for axis in range(3)]
