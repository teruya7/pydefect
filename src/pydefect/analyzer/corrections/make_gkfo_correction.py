...
def make_gkfo_correction(efnv_correction: ExtendedFnvCorrection,
    ...
    Args:
        efnv_correction: EFNV correction of initial state.
        additional_charge: Charge change (+1 or -1).
        final_calc_results: CalcResults of final state.
        initial_calc_results: CalcResults of initial state.
        diele_tensor: Full dielectric tensor.
        ion_clamped_diele_tensor: Electronic dielectric tensor.
    ...
    for index, site in enumerate(initial_calc_results.structure):
        ...
        rel_coord = [site_coord - ref_coord
                     for site_coord, ref_coord in zip(site.frac_coords, defect_coords)]
