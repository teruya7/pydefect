...
import numpy as np
from pydefect.analyzer.band_edge.band_edge_states import BandEdgeOrbitalInfos, \
...
def get_similar_orb_idx(orbs: List[OrbitalInfo],
    ...
    Returns:
        Tuple of (band_index, orbital_difference).

    Raises:
        ValueError: If no similar orbital is found.
...
def get_localized_orbs(orb_info_by_spin: List[List[OrbitalInfo]],
    ...
    result = []
    for band_idx, orbs_by_kpt in enumerate(orbs_by_band_by_kpt, loc_band_index_range[0]):
        def get_ave(key):
            try:
                return sum([getattr(orbital_info, key) * weight 
                           for orbital_info, weight in zip(orbs_by_kpt, weights)])
            except TypeError:
                return None

        orbs = defaultdict(list)
        for orbital_info, weight in zip(orbs_by_kpt, weights):
            for element, contribution in orbital_info.orbitals.items():
                for orb_idx, coeff in enumerate(contribution):
                    try:
                        orbs[element][orb_idx] += coeff * weight
                    except IndexError:
                        orbs[element].append(0.0)
                        orbs[element][orb_idx] += coeff * weight
...
def make_band_edge_states(orbital_infos: BandEdgeOrbitalInfos,
    ...
    Determines VBM, CBM, and localized states for defect calculations.
...
def orbital_diff(orbital_1: dict, orbital_2: dict) -> float:
    ...
    for element in element_set:
        result += sum([abs(val1 - val2) for val1, val2
                       in zip_longest(orb_1[element], orb_2[element], fillvalue=0)])
